...
class DeviceViewSet(ConfigContextQuerySetMixin, NautobotModelViewSet):
    ...
    def napalm(self, request, pk):
        ...
        if device.secrets_group:
            try:
                driver_name = (getattr(device.platform, "napalm_driver", "") or "").lower()

                # Map driver → argument key without inline literals triggering S105
                ENABLE_ARG_KEYS = {
                    "default": str("secret"),
                    "eos": str("enable_password"),
                }
                enable_password_arg = ENABLE_ARG_KEYS.get(driver_name, ENABLE_ARG_KEYS["default"])
